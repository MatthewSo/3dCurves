  ...
  while ( fgets(line, sizeof(line), f) != NULL ) {
	...
	tmp = make_rotZ( theta );
      ...
      *strchr(line, '\n') = 0;
      ...
      fgets(line, sizeof(line), f);
      sscanf(line, "%lf %lf %lf %lf",xvals, yvals, yvals+1,xvals+1);
      add_sphere(edges, xvals[0], yvals[0], yvals[1], xvals[1], .01);      
    }

    else if ( strncmp(line, "torus", strlen(line)) == 0 ) {
      fgets(line, sizeof(line), f);
      sscanf(line, "%lf %lf %lf %lf %lf", xvals, yvals, xvals+1, yvals+1, xvals+2);
      add_torus(edges, xvals[0], yvals[0], xvals[1], yvals[1],xvals[2], .01);      
    }
